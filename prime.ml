...
let rec prime_tbl m n   =  
        if m>n then [] else 
        if is_prime m then m::prime_tbl(m+1)n else prime_tbl (m+1) n;;

let print_intlist l = "["^(foldr(fun x->(^)(soi x^"; "))"]" l) ;;
let print_primes l = foldr(fun x->(^)(soi x^";")) "" l ;;

let mk_prime_tbl m n = 
    let tbl         = "prime_tbl_"^soi m^"_"^soi n in 
    let fn          = "prime/" ^ tbl ^ ".ml" in
    let fd          = open_out fn in 
    let print_start = output_bytes fd ("let "^tbl^" = [") in
    let loop m n fd = output_bytes fd (print_primes (prime_tbl m n)) in
    let size        = (n-m-1)/10 in
    let m'          = (m-1)/size in let n' = n/size in 
    let iter        = rev (m' -- n') in
    let ()= foldr (fun x xs->let()=loop(x*size+1)((x+1)*size)fd in xs)() iter in 
    let print_end   = output_bytes fd "];;" in
    close_out fd;;  
