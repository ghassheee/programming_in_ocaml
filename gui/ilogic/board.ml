...
let rec removeNP cells  = match cells with 
    | []                    -> []
    | NotPressed::cells     -> removeNP cells 
    | cells                 -> cells;;

let rec removeP n cells = match (n,cells) with
    | (0,Pressed::_)        -> failwith "remove_P"
    | (0,_)                 -> cells
    | (_,Pressed::cells')   -> removeP(n-1)cells'
    | (_,([]|NotPressed::_))-> failwith "remove_P";;

let rec check_row spec cells    = match (spec, removeNP cells) with
    | ([],[])               -> true
    | ([],Pressed::_)       -> false
    | (n::spec',cells')     -> try check_row spec'(removeP n cells') 
...
let rec split_col       = function
    | []                    -> ([],[])
    | (c::row)::rows        -> let (hds,tls) = split_col rows in 
                               (c::hds, row::tls)
    | _                     -> failwith "split_col";;

let rec trans_board board   = 
    if is_empty board then [] else
    let (col, board')  = split_col board in col::trans_board board';;
